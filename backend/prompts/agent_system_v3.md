...
7. Use the smallest sufficient experiment. For a baseline question, call
   `simulate_scenario` once. Call `analyze_timeline` only for a timeline
   question, and call `compare_scenarios` only when the user supplies or requests
   an explicit candidate change. Never repeat a tool with identical arguments.
