...
pub const YOU_FENG_DIE_YU_SHI_WEI_IDS: &[u32] = &[
    44121, 44154, 44187, 44220, 44253, 44286,
];

/// 幽烽蝶语·不归（防御老小橙武，6 把）—— 当前无 PVE 输出特效，仅占位
pub const YOU_FENG_DIE_YU_BU_GUI_IDS: &[u32] = &[
    44122, 44155, 44188, 44221, 44254, 44287,
];
...
pub const CY_DPS_T_SET_IDS: &[u32] = &[
    // 5936 西塞·行歌（22500）
    101022, 101053, 101084, 101115, 101146,
    // 6481 孤漠·雁际（25900）
...
pub const CY_JJC_SET_IDS: &[u32] = &[
    ...
    102853, 102880, 102907, 102961, 102988,
    103339, 103366, 103393, 103447, 103474,
    // 6516 剑意狂歌·景盛（29800/31200）
    105829, 105856, 105883, 105937, 105964,
    105991, 106018, 106045, 106099, 106126,
    // 6817 漫天飞羽·锋戎（34800/36500/40800/42800）
    107897, 107925, 107953, 108009, 108037,
    108065, 108093, 108121, 108177, 108205,
    110358, 110386, 110414, 110470, 110498,
    110526, 110554, 110582, 110638, 110666,
];
...
pub const CY_WEI_WANG_SET_IDS: &[u32] = &[
    ...
    102529, 102556, 102583, 102637, 102664,
    103015, 103042, 103069, 103123, 103150,
    // 6356 雷鸣风怒·关寒（25900/27100）
    102691, 102718, 102745, 102799, 102826,
    103177, 103204, 103231, 103285, 103312,
    // 6544 覆海惊澜·重楼（29800/31200）
    105181, 105208, 105235, 105289, 105316,
    105505, 105532, 105559, 105613, 105640,
    // 6572 焚野烬风·刃寒（29800/31200）
    105343, 105370, 105397, 105451, 105478,
    105667, 105694, 105721, 105775, 105802,
    // 6845 霁月玄霜·代春 / 天风霄汉·塞北（34800~42800）
    107225, 107253, 107281, 107337, 107365,
    107561, 107589, 107617, 107673, 107701,
    109686, 109714, 109742, 109798, 109826,
    110022, 110050, 110078, 110134, 110162,
    // 6873 魇雾苍火·替冬 / 地火冥幽·惊敌（34800~42800）
    107393, 107421, 107449, 107505, 107533,
    107729, 107757, 107785, 107841, 107869,
    109854, 109882, 109910, 109966, 109994,
    110190, 110218, 110246, 110302, 110330,
];
...
pub const CY_GUARDIAN_T_SET_IDS: &[u32] = &[
    // 5937 西塞·旋草（22500）
    101023, 101054, 101085, 101116, 101147,
    // 6482 孤漠·远别（25900）
...
pub fn shen_bing_wu_shuang_for(weapon_id: u32) -> Option<(u32, f64)> {
    SHEN_BING_WU_SHUANG_STRAIN_TABLE.iter().enumerate()
...
use crate::{Player, ScriptEmitter, AttribSlots,
    BUFF_FUMO_BODONG, BUFF_FU_DA_DPS_HAT, BUFF_FU_DA_DPS_YI, BUFF_FU_DA_T_HAT, BUFF_FU_DA_T_WRIST,
    ...
    BUFF_YE_PANTS_MULTI_RATE, BUFF_YE_PANTS_MULTI_CR, BUFF_YE_PANTS_MULTI_OV,
...
pub fn on_battle_start(player: &mut Player) {
    ...
    let cur_overcome = base.overcome_level
        + slots.get(&PhysicsOvercomeBase).copied().unwrap_or(0.0);
    let cur_crit = base.crit_level
        + slots.get(&PhysicsCriticalStrike).copied().unwrap_or(0.0);
    let cur_surplus = base.surplus_value
        + slots.get(&SurplusValueBase).copied().unwrap_or(0.0);
    // strain rate（用于 裤·高级 conditional）：
    let strain_pct = slots.get(&StrainBasePercentAdd).copied().unwrap_or(0.0) / 1024.0;
    let cur_strain_lvl = base.strain_level
        + slots.get(&StrainBase).copied().unwrap_or(0.0);
    ...
    if player.has_enchant(YE_NECKLACE_OVERCOME_TO_ATTACK) {
        let stacks = ((cur_overcome / YE_NECK_CRIT_THRESHOLD).floor() as u32).min(YE_NECK_MAX_STACKS);
        for _ in 0..stacks {
...
pub fn on_post_cast(player: &mut Player, em: &mut ScriptEmitter, skill: &crate::SkillSpec, t: f64) {
    ...
    matches!(skill.skill_id, 38984 | 38985 | 40789 | 38966 | 42837)
        .then(|| ()) // no-op
        .map(|_| { return; });
    if matches!(skill.skill_id, 38984 | 38985 | 40789 | 38966 | 42837) { return; }
    ...
    if player.has_enchant(ENCHANT_WRIST_AYQJ) {
        ...
        if cd_ready && player.accum_equip_effect(38984, ENCHANT_WRIST_PROB, 1024) {
            player.active_cds.insert(cd_key.to_string(), t + ENCHANT_WRIST_INNER_CD);
    ...
    if player.has_enchant(ENCHANT_SHOES_AYQJ) {
        ...
        if cd_ready {
            ...
            if player.accum_equip_effect(38985, crit_prob_1024.max(0), 1024) {
                player.active_cds.insert(cd_key.to_string(), t + ENCHANT_SHOES_INNER_CD);
                em.emit("刃凌", 38985, t);
            }
        }
    }
    ...
    if player.has_enchant(YE_SHOES_CRIT_6000) {
        let cd_key = "cd_ye_鞋会";
        if player.active_cds.get(cd_key).copied().unwrap_or(0.0) <= t {
            player.active_cds.insert(cd_key.to_string(), t + YE_SHOES_INNER_CD);
    ...
    if player.has_enchant(YE_WRIST_TRUE_DMG) {
        let cd_key = "cd_ye_护手";
        if player.active_cds.get(cd_key).copied().unwrap_or(0.0) <= t {
            player.active_cds.insert(cd_key.to_string(), t + YE_WRIST_INNER_CD);
            em.emit("巽", 40789, t);
        }
    }
    ...
    if player.has_enchant(YE_DART_TRUE_DMG) {
        let cd_key = "cd_ye_暗器";
        if player.active_cds.get(cd_key).copied().unwrap_or(0.0) <= t {
            player.active_cds.insert(cd_key.to_string(), t + YE_DART_INNER_CD);
    ...
    if player.has_enchant(YE_BELT_HASTE_TRUE) {
        ...
        if player.active_cds.get(cd_key).copied().unwrap_or(0.0) <= t {
            player.active_cds.insert(cd_key.to_string(), t + YE_BELT_HASTE_INNER_CD);
            em.emit("速·震", 42837, t);
        }
    }
}
