...
use crate::macro_engine::{MacroConfig, MacroAction};
...
impl StructCtx {
    ...
    pub fn build_param_table(pool: &RulePool) -> (Vec<(String, usize)>, Vec<EffectiveTunable>) {
        ...
        let push_rule = |rid: &str,
            ...
            let Some(rule) = pool.rules.get(rid) else { return; };
...
impl StructIndividual {
    pub fn seed(ctx: &StructCtx) -> Self {
        let shield_order: Vec<(String, bool)> = ctx.pool.shield_default_order.iter()
            .map(|id| (id.clone(), ctx.pool.shield_default_enabled.contains(id)))
            .collect();
        let blade_order: Vec<(String, bool)> = ctx.pool.blade_default_order.iter()
            .map(|id| (id.clone(), ctx.pool.blade_default_enabled.contains(id)))
            .collect();
        let values: Vec<f64> = ctx.tunables.iter().map(|t| t.clamp_snap(t.param.original)).collect();
        Self { shield_order, blade_order, values, fitness: 0.0, stats: EvalStats::default() }
    }
    ...
    pub fn half_random(ctx: &StructCtx, rng: &mut impl Rng) -> Self {
        ...
        for (id, en) in ind.shield_order.iter_mut() {
            let Some(rule) = ctx.pool.rules.get(id) else { continue; };
            if rule.locked { *en = true; continue; }
    ...
    pub fn full_random(ctx: &StructCtx, rng: &mut impl Rng) -> Self {
        let mut shield_order: Vec<(String, bool)> = ctx.pool.shield_default_order.iter()
            .map(|id| (id.clone(), false))
            .collect();
        let mut blade_order: Vec<(String, bool)> = ctx.pool.blade_default_order.iter()
            .map(|id| (id.clone(), false))
            .collect();
...
pub fn run_once(ind: &StructIndividual, ctx: &StructCtx, duration: f64) -> f64 {
    ...
    let (timeline, _debug) = simulate_macro(
        &rendered.config, &mut player, &skill_map,
...
fn tournament<'a>(pop: &'a [StructIndividual], k: usize, rng: &mut impl Rng) -> &'a StructIndividual {
    tournament_by(pop, k, rng, &|ind: &StructIndividual| ind.fitness)
}

fn tournament_by<'a>(pop: &'a [StructIndividual], k: usize, rng: &mut impl Rng, key: &dyn Fn(&StructIndividual) -> f64) -> &'a StructIndividual {
...
fn crossover(
    ...
    if !rng.gen_bool(rate) { return p1.clone(); }
    ...
    let values: Vec<f64> = p1.values.iter().zip(p2.values.iter()).enumerate()
        .map(|(i, (&x, &y))| ctx.tunables[i].clamp_snap(alpha * x + (1.0 - alpha) * y))
        .collect();

    StructIndividual { shield_order, blade_order, values, fitness: 0.0, stats: EvalStats::default() }
}
...
fn ox_cross(
    ...
    let fill_list: Vec<(String, bool)> = p2.iter()
        ...
        if slot.is_none() {
            if let Some((id, _en_p2)) = fill_iter.next() {
                ...
                let en = if rng.gen_bool(0.7) {
                    p1.iter().find(|(i, _)| i == &id).map(|(_, e)| *e).unwrap_or(false)
                } else {
                    p2.iter().find(|(i, _)| i == &id).map(|(_, e)| *e).unwrap_or(false)
                };
                used.insert(id.clone());
                *slot = Some((id, en));
            }
    ...
    child.into_iter().map(|o| o.unwrap_or_else(|| (String::new(), false))).collect()
}
...
fn bubble_swap(order: &mut Vec<(String, bool)>, pool: &RulePool, rng: &mut impl Rng) {
    ...
    for _ in 0..n {
        ...
        let j = i + 1;
        if is_locked(&order[i].0, pool) || is_locked(&order[j].0, pool) { continue; }
        order.swap(i, j);
        return;
    }
}
...
fn toggle_enabled(order: &mut Vec<(String, bool)>, pool: &RulePool, rng: &mut impl Rng) {
    ...
    for _ in 0..n {
        let i = rng.gen_range(0..n);
        if is_locked(&order[i].0, pool) { continue; }
        order[i].1 = !order[i].1;
        return;
    }
}
...
fn cross_page_move(ind: &mut StructIndividual, ctx: &StructCtx, rng: &mut impl Rng) {
    // 找所有 Either 规则
    let either_ids: Vec<String> = ctx.pool.rules.iter()
        .filter(|(_, r)| r.page == PageTag::Either && !r.locked)
        .map(|(k, _)| k.clone())
        .collect();
    if either_ids.is_empty() { return; }
...
impl<'a> StructGaRun<'a> {
    pub fn run(&mut self) -> (Option<StructIndividual>, f64, usize) {
        ...
        if let Some(ref bi) = best_ind {
            let rendered = render_individual(
                &self.ctx.pool, &bi.shield_order, &bi.blade_order,
                &self.ctx.param_keys, &bi.values,
            );
        ...
        for gen in 0..self.params.generations {
            ...
            let best_values = population.iter().max_by(|a, b| sort_key(a).partial_cmp(&sort_key(b)).unwrap_or(std::cmp::Ordering::Equal))
            ...
            (self.sink)(GaEvent::Progress {
                gen, best: stats.best, avg: stats.avg, worst: stats.worst,
                diversity: stats.diversity, best_values,
            ...
            while new_pop.len() < pop_size {
                let a = tournament_by(&population, self.params.tournament_k, &mut rng, &sort_key).clone();
                let b = tournament_by(&population, self.params.tournament_k, &mut rng, &sort_key).clone();
            ...
            if let Some(gb) = gen_best {
                ...
                if gb_score > best_score + 1e-6 {
                    best_score = gb_score;
                    let rendered = render_individual(
                        &self.ctx.pool, &gb.shield_order, &gb.blade_order,
                        &self.ctx.param_keys, &gb.values,
                    );
        ...
        for (i, ind) in top.iter().enumerate() {
            let rendered = render_individual(
                &self.ctx.pool, &ind.shield_order, &ind.blade_order,
                &self.ctx.param_keys, &ind.values,
            );
            ...
            if let Ok(rel) = self.archive.write_topn(i + 1, canonical, &cfg) {
                entries.push(TopNEntry {
                    ...
                    values: ind.values.clone(), macro_text,
...
pub struct Stats { pub best: f64, pub avg: f64, pub worst: f64, pub diversity: f64 }
...
fn compute_stats(pop: &[StructIndividual]) -> Stats {
    ...
    let diversity = if pairs == 0 { 0.0 } else { total / pairs as f64 };
    Stats { best, avg, worst, diversity }
}

fn enabled_set_distance(a: &StructIndividual, b: &StructIndividual) -> f64 {
    let ea: HashSet<&str> = a.shield_order.iter().chain(a.blade_order.iter())
        .filter(|(_, en)| *en).map(|(id, _)| id.as_str()).collect();
    let eb: HashSet<&str> = b.shield_order.iter().chain(b.blade_order.iter())
        .filter(|(_, en)| *en).map(|(id, _)| id.as_str()).collect();
...
fn struct_approx_eq(a: &StructIndividual, b: &StructIndividual) -> bool {
    if a.shield_order != b.shield_order { return false; }
    if a.blade_order != b.blade_order { return false; }
    a.values.iter().zip(b.values.iter()).all(|(x, y)| (x - y).abs() < 1e-3)
}
...
fn build_macro_text_struct(rendered: &RenderResult) -> String {
    ...
    let shield_text = rendered.config.pages.iter()
        .find(|p| p.stance_filter == Some(Stance::Shield))
        .map(|p| serialize_lines(&p.lines)).unwrap_or_default();
    let blade_text = rendered.config.pages.iter()
...
pub fn compute_struct_diff(ind: &StructIndividual, pool: &RulePool, rendered: &RenderResult) -> StructDiff {
    let mut new_rules_enabled = Vec::new();
    let mut original_disabled = Vec::new();
    let emitted_set: HashSet<&str> = rendered.shield_emitted.iter()
        .chain(rendered.blade_emitted.iter())
        .map(|s| s.as_str()).collect();
    // 扫描 individual 的 order
    for (id, en) in ind.shield_order.iter().chain(ind.blade_order.iter()) {
        let Some(rule) = pool.rules.get(id) else { continue; };
...
pub fn render_and_diff(ind: &StructIndividual, ctx: &StructCtx) -> (RenderResult, StructDiff) {
    let r = render_individual(&ctx.pool, &ind.shield_order, &ind.blade_order, &ctx.param_keys, &ind.values);
