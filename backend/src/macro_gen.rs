...
pub struct InputBuff {
    pub name: String,
    #[serde(default)] pub remaining: f64,
    #[serde(default)] pub stacks: u32,
}
...
pub struct InputState {
    #[serde(default)] pub rage: i32,
    #[serde(default)] pub stance: Stance,
    #[serde(default)] pub buffs: Vec<InputBuff>,
    #[serde(default)] pub target_buffs: Vec<InputBuff>,
    #[serde(default)] pub skill_cds: Vec<InputSkillCd>,
}

#[derive(Debug, Deserialize)]
pub struct InputCast {
    pub name: String,
    #[serde(default)] pub triggered: bool,
    #[serde(default = "default_is_main")] pub is_main: bool,
    #[serde(default)] pub state_before: Option<InputState>,
}
fn default_is_main() -> bool { true }

#[derive(Debug, Deserialize, Default, Clone)]
pub struct GenOptions {
    #[serde(default = "default_page_chars")] pub max_chars_per_page: usize,
    #[serde(default = "default_max_terms")] pub max_terms: usize,
    #[serde(default = "default_max_clauses")] pub max_clauses: usize,
    #[serde(default = "default_min_gain")] pub min_gain: f64,
    #[serde(default = "default_min_cover")] pub min_cover_pct: f64,
...
fn default_page_chars() -> usize { 128 }
fn default_max_terms() -> usize { 2 }
fn default_max_clauses() -> usize { 3 }
fn default_min_gain() -> f64 { 0.05 }
fn default_min_cover() -> f64 { 1.0 }
fn default_consistency_skills() -> Vec<String> {
    vec!["血怒".into(), "业火麟光".into()]
}
fn default_consistency_gain_mult() -> f64 { 3.0 }
fn default_consistency_terms_bonus() -> usize { 2 }
fn default_consistency_priority() -> f64 { 5.0 }
...
pub fn generate(timeline: &[InputCast], opts: &GenOptions) -> GenResult {
    ...
    for e in timeline {
        ...
        let Some(st) = e.state_before.as_ref() else { continue; };
        ...
        if e.is_main { last_main = Some(base); }
    }
    ...
    for s in &samples {
        by_stance.entry(stance_key(s.state.stance)).or_default().push(s);
    }
    ...
    if opts.use_v2 {
        ...
        for (sk, casts) in &by_stance {
            ...
            let mut best_result: Option<(String, usize, Vec<DroppedRule>, Vec<String>, Vec<RuleDiagnostic>, f64)> = None;
            for trial in 0..n_starts {
                ...
                if best_result.as_ref().map_or(true, |(_, _, _, _, _, bs)| score > *bs) {
    ...
    let cross_page_skills = if opts.use_v2 {
        ...
        for d in &all_diagnostics {
            skill_pages.entry(d.skill.clone()).or_default().insert(d.stance.clone());
        }
        skill_pages.into_iter()
...
fn build_page<'a>(
    ...
    opts: &GenOptions,
    stance_label: &str,
) -> (String, usize, Vec<DroppedRule>, Vec<String>, Vec<RuleDiagnostic>) {
    // 按基础名聚类
    ...
    struct Rule {
        ...
        #[allow(dead_code)] cover: usize,
        n_cond: usize,
        #[allow(dead_code)] is_main: bool,
        copeland: i32,
        off_gcd: bool,
    }
    ...
    for s in page_samples {
        if !s.is_main { continue; } // 非主 GCD 不竞争槽位
        let x = &s.skill_base;
        for (y, _) in &by_base {
            if y == x { continue; }
            let y_main = by_base[y].iter().filter(|s2| s2.is_main).count() * 2
                >= by_base[y].len();
            if !y_main { continue; }
            let mr = *min_rage.get(y).unwrap_or(&0);
            if !skill_feasible(y, s.state) { continue; }
            if s.state.rage < mr { continue; }
            *wins.entry(x.clone()).or_default().entry(y.clone()).or_insert(0) += 1;
        }
    }
    let copeland_score = |skill: &str| -> i32 {
        ...
        for other in by_base.keys() {
            ...
            let a = wins.get(skill).and_then(|m| m.get(other)).copied().unwrap_or(0) as i32;
            let b = wins.get(other).and_then(|m| m.get(skill)).copied().unwrap_or(0) as i32;
            sc += a - b;
        }
        sc
    };
    ...
    for (skill, positives) in &by_base {
        ...
        if off_gcd && opts.use_off_gcd_treatment {
            for s in page_samples {
                if s.skill_base == *skill { continue; }
                if !skill_feasible(skill, s.state) { continue; }
                negatives.push(*s);
            }
        } else {
            for (other, ns) in &by_base {
                if other == skill { continue; }
                for s in ns {
                    if s.is_main != skill_is_main { continue; }
                    if !skill_feasible(skill, s.state) { continue; }
                    negatives.push(s);
                }
            }
        ...
        let cond = if negatives.is_empty() {
            ...
            extract_rule(positives, &negatives, opts, min_gain, allow_partial, max_terms, max_clauses)
        };
        let n_cond = cond.as_ref().map(count_atoms).unwrap_or(0);
        let cp = if opts.use_copeland_priority { copeland_score(skill) } else { 0 };

        // ── 诊断：top-N 候选 + 最终 F1 ──
        let final_f1 = cond.as_ref()
            .map(|c| score_f1(c, positives, &negatives))
            .unwrap_or(0.0);
        let cond_text = cond.as_ref()
        ...
        let mut cand_scored: Vec<(String, f64, bool)> = enumerate_candidates(positives, &negatives, opts.use_v2)
            .into_iter()
            .map(|c| {
        ...
        let candidates_top = cand_scored.into_iter()
            .map(|(expr, f1, sel)| CandidateInfo { expr, f1, selected: sel })
            .collect();
    ...
    // 渲染规则为文本行
    let rendered: Vec<String> = rules.iter().map(|r| {
        match &r.cond {
            Some(c) => {
    ...
    let selected = if opts.use_v2 && rendered.iter().map(|l| l.chars().count()).sum::<usize>() > opts.max_chars_per_page {
...
fn collect_atom_exprs(cond: &MacroCondition, out: &mut BTreeSet<String>) {
    match cond {
        ...
        _ => { out.insert(cond.display_string()); }
    }
}
...
fn collect_bufftime_atoms(cond: &MacroCondition, out: &mut BTreeSet<(String, bool)>) {
    match cond {
        ...
        MacroCondition::BuffTime(n, _, _) => { out.insert((n.clone(), false)); }
        MacroCondition::TBuffTime(n, _, _) => { out.insert((n.clone(), true)); }
        _ => {}
    }
}
...
fn extract_rule<'a>(
    ...
    //     才真正追加
    let mut bufftime_buff_names: std::collections::BTreeSet<(String, bool)> = std::collections::BTreeSet::new();
    for cl in &clauses {
...
fn guard_virtual_weak(cond: MacroCondition) -> MacroCondition {
    match cond {
        ...
        MacroCondition::TnoBuff(ref name) if name == "虚弱" => {
            MacroCondition::And(
...
fn greedy_and<'a>(
    ...
    for _ in 0..max_terms {
        ...
        if opts.use_v2 && cands.len() >= 2 {
            // 先找 top-K 单条件
            let mut singles: Vec<(MacroCondition, f64)> = cands.iter()
        ...
        let Some((c, s)) = best else { break; };
        if s < min_gain { break; }
        pos.retain(|st| eval_cond(&c, st));
        neg.retain(|st| eval_cond(&c, st));
        conds.push(c);
        if neg.is_empty() { break; }
    }
    if conds.is_empty() { return None; }
    let mut it = conds.into_iter();
    let first = it.next().unwrap();
    Some(it.fold(first, |acc, c| MacroCondition::And(Box::new(acc), Box::new(c))))
}
...
fn enumerate_candidates<'a>(pos: &[&Sample<'a>], neg: &[&Sample<'a>], use_v2: bool) -> Vec<MacroCondition> {
    let mut out = Vec::new();
    ...
    // —— 怒气：分位 + 固定点（v2: 加入所有正/负样本实际值）——
    {
        ...
        for &q in &[25, 35, 45, 55, 65, 75] { ts.insert(q); }
        for v in quantiles_i32(&pr, &[0.25, 0.5, 0.75]) { ts.insert(v); }
        for v in quantiles_i32(&nr, &[0.25, 0.5, 0.75]) { ts.insert(v); }
        if use_v2 {
            // 枚举所有正/负样本出现过的怒气值作为候选阈值
            for &v in &pr { ts.insert(v); }
            for &v in &nr { ts.insert(v); }
    ...
    for s in pos.iter().chain(neg.iter()) {
        for b in &s.state.buffs { self_names.insert(b.name.clone()); }
    }
    for n in &self_names {
    ...
    for s in pos.iter() {
        for b in &s.state.buffs { self_times.entry(b.name.clone()).or_default().push(b.remaining); }
    }
    if use_v2 {
        for s in neg.iter() {
            for b in &s.state.buffs { self_times.entry(b.name.clone()).or_default().push(b.remaining); }
    ...
    for (name, times) in self_times {
        ...
        for &t in &[1.0f64, 2.0, 3.0, 5.0] { ths.insert(t.round() as i64); }
        for v in qs { ths.insert(v.round() as i64); }
        if use_v2 {
            // 枚举正/负样本中实际出现的时间值（取整）
            for &v in &times {
            ...
            if ths.len() > 20 {
                ...
                ths = BTreeSet::new();
                for &t in &[1i64, 2, 3, 5] { ths.insert(t); }
                if step > 0 {
                    for i in (0..sorted.len()).step_by(step.max(1)) { ths.insert(sorted[i]); }
                }
            }
        }
        for t in ths {
    ...
    for s in pos.iter().chain(neg.iter()) {
        for b in &s.state.target_buffs { tgt_names.insert(b.name.clone()); }
    }
    for n in &tgt_names {
    ...
    for s in pos.iter() {
        for b in &s.state.target_buffs { tgt_times.entry(b.name.clone()).or_default().push(b.remaining); }
    }
    if use_v2 {
        for s in neg.iter() {
            for b in &s.state.target_buffs { tgt_times.entry(b.name.clone()).or_default().push(b.remaining); }
    ...
    for s in pos.iter().chain(neg.iter()) {
        if let Some(ls) = &s.last_main_base { last_names.insert(ls.clone()); }
    }
    for n in last_names {
...
fn eval_cond<'a>(c: &MacroCondition, s: &Sample<'a>) -> bool {
    ...
    match c {
        ...
        MacroCondition::BuffTime(n, op, v) =>
            st.buffs.iter().find(|b| &b.name == n)
        ...
        MacroCondition::TBuffTime(n, op, v) =>
            st.target_buffs.iter().find(|b| &b.name == n)
                .map_or(false, |b| op.compare_f64(b.remaining, *v)),
...
fn unkey_stance(k: u8) -> Stance {
    match k { 1 => Stance::Shield, 2 => Stance::Blade, 3 => Stance::Wall, _ => Stance::Any }
}
fn stance_label(s: Stance) -> &'static str {
    match s {
        Stance::Shield => "shield", Stance::Blade => "blade", Stance::Wall => "wall", _ => "any",
    }
}
...
fn detect_steady_start(samples: &[Sample]) -> usize {
    if samples.len() < 8 { return 0; }
    let first_stance = samples.iter()
        .find(|s| s.is_main)
        .map(|s| stance_key(s.state.stance));
    let Some(first_st) = first_stance else { return 0; };
    ...
    for (i, s) in samples.iter().enumerate() {
        ...
        if sk != first_st { left_first = true; }
        // 条件：离开过初始姿态 → 回来 + 怒气 ≥ 50
    ...
    for (i, s) in samples.iter().enumerate() {
        if s.is_main { counted += 1; }
        if counted >= skip_main { return i + 1; }
    }
    0
}
