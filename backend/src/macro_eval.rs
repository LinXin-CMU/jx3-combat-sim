...
use serde::Serialize;
use crate::{
    Player, SkillSpec, CastEvent, RecipeEntry,
    frames_to_sec,
    macro_engine::*,
};
...
fn eval_condition(cond: &MacroCondition, state: &Phase1State) -> bool {
    ...
    let _guard = crate::scopeguard_perf(
        |ns| crate::perf_add(|p| { p.macro_cond_n += 1; p.macro_cond_ns += ns; }),
        _t0
    );
    match cond {
        MacroCondition::Rage(op, val) => {
            op.compare_i32(state.player.rage, *val)
        }
        ...
        MacroCondition::NoBuff(name) => {
            buff_name_to_id(name)
                .map(|id| !state.buff_lookup.contains_key(&id))
                .unwrap_or(true)
        }
        MacroCondition::BuffStack(name, op, val) => {
            buff_name_to_id(name)
                .map(|id| {
                    let stacks = state.buff_lookup.get(&id)
        ...
        MacroCondition::BuffTime(name, op, val) => {
            ...
            buff_name_to_id(name)
                ...
                .map(|idx| {
                    ...
                    let r = if inst.expires_at == 0.0 { f64::MAX }
                            else { (inst.expires_at - state.player.current_time).max(0.0) };
                    op.compare_f64(r, *val)
                })
                .unwrap_or(false)
        }
        MacroCondition::TBuff(name) => {
            buff_name_to_id(name)
                .map(|id| state.target_buff_lookup.contains_key(&id))
                .unwrap_or(false)
        }
        MacroCondition::TnoBuff(name) => {
            buff_name_to_id(name)
                .map(|id| !state.target_buff_lookup.contains_key(&id))
                .unwrap_or(true)
        }
        MacroCondition::TBuffTime(name, op, val) => {
            buff_name_to_id(name)
        ...
        MacroCondition::SkillNotInCd(name) => {
            ...
            let ranks = state.skill_map.get(name.as_str())
        ...
        MacroCondition::LastSkill(name) => {
            ...
            if let Some(last) = state.last_skill.as_deref() {
                if last == name.as_str() { return true; }
                // 数字 ID：查技能名比较
        ...
        MacroCondition::LastSkillNot(name) => {
            if let Some(last) = state.last_skill.as_deref() {
                if last == name.as_str() { return false; }
                if let Some(id) = name.parse::<u32>().ok() {
                    if let Some(spec) = state.skill_by_id.get(&id) {
                        let base = spec.name.split('·').next().unwrap_or(&spec.name);
                        return last != base;
        ...
        MacroCondition::And(a, b) => {
            eval_condition(a, state) && eval_condition(b, state)
        }
...
pub fn evaluate_phase2<'a>(
    ...
    macro_rules! push_dbg {
        ($skill:expr, $castable:expr, $reason:expr) => {
            if enable_debug {
    ...
    for entry in pool {
                ...
                if let Some(id) = entry.skill_name.parse::<u32>().ok() {
                    if let Some(spec) = skill_by_id.get(&id) {
                        ...
                        if let Some(r) = skill_map.get(base) {
                            ...
                            push_dbg!(entry.skill_name.clone(), false,
                    ...
                    push_dbg!(entry.skill_name.clone(), false, "未知技能".into());
                    continue;
                }
            }
        };
        ...
        let skill = match player.pick_rank(ranks) {
            Some(s) => s,
            None => {
                push_dbg!(entry.skill_name.clone(), false, "体态/条件不满足".into());
                continue;
            }
        };
        ...
        {
            push_dbg!(entry.skill_name.clone(), false, "战绝：仅可释放苍雪刀招式".into());
            continue;
        }

        // combo_follow：若本技能会重定向到子技能，用子技能判断 CD 就绪
        let effective_skill = crate::resolve_combo_follow(skill, player, skill_by_id)
            .unwrap_or(skill);
        ...
        if player.channel_end > player.current_time + 0.001 {
            if entry.is_fcast {
                ...
                push_dbg!(entry.skill_name.clone(), false, "引导中 (非fcast)".into());
                continue;
            }
        }

        push_dbg!(entry.skill_name.clone(), true, "可释放".into());

        return (Some(Phase2Result {
            skill,
...
pub(crate) fn skill_is_main(skill: &SkillSpec) -> bool {
    ...
    skill.cooldowns.iter().any(|cd| {
        (cd.cd_id == "gcd_1.0" || cd.cd_id == "gcd_1.5") &&
        cd.mode == CdMode::CheckAndTrigger
    })
}
...
pub fn execute_cast(
    ...
    crate::fill_tick_events(&mut tick_events, ctx.skill_by_id, ctx.dmg_ctx, ctx.recipes_table, player);
    out.extend(tick_events);
    *prev_time = est_time;
    player.current_time = est_time;

    // 2. 释放
    let state_before = if player.lite_mode { None } else { Some(crate::snapshot_event_state(player)) };
    let rage_before = player.rage;
    let channel_override = if actual_skill.channel_interval.is_some() { Some(u32::MAX) } else { None };
    ...
    if player.current_time > *prev_time {
        ...
        if !gap_events.is_empty() {
            crate::fill_tick_events(&mut gap_events, ctx.skill_by_id, ctx.dmg_ctx, ctx.recipes_table, player);
        }
    ...
    let (cast_time, _cd_wait, ch_ticks, ch_max, ch_dur, applied_offset, ret_max_offset) =
        match cast_result {
            Some(r) => r,
            None => return CastOutcome { events: out, cast_success: false },
        };

    if skill_is_main(actual_skill) { *is_first_main = false; }
    if !actual_skill.passive { player.start_swing(cast_time); }
    ...
    let (dmg, dmg_normal, dmg_crit, dmg_total, rt_snap) = if let Some((a, t)) = ctx.dmg_ctx {
        ...
        let (r, dt, rt) = crate::calc_event_damage(actual_skill, a, t, player, &runtime_recipes, ctx.recipes_table, ticks);
    ...
    if actual_skill.skill_id == 13054 {
        if let Some((a, _)) = ctx.dmg_ctx {
            ...
            if let Some(inst) = player.target_buffs.iter_mut().find(|b| b.buff_id == crate::BUFF_LIU_XUE) {
    ...
    let (event_name, final_dmg, final_dmg_n, final_dmg_c, final_dmg_t, final_rt, eff_recipes) =
        if let Some((ref ov_name, ov_id)) = primary_override {
                ...
                (ctx.dmg_ctx, ctx.skill_by_id.get(&ov_id).copied())
            {
                ...
                let (r, dt, rt2) = crate::calc_event_damage(ov_spec, a, t_cfg, player, &ov_recipes, ctx.recipes_table, ticks);
            ...
            (actual_skill.name.clone(), dmg, dmg_normal, dmg_crit, dmg_total, rt_snap, runtime_recipes.clone())
        };
    ...
    out.push(CastEvent {
        ...
        state_before,
        state_after: if player.lite_mode { None } else { Some(crate::snapshot_event_state(player)) },
        damage: final_dmg,
        damage_normal: final_dmg_n,
        damage_crit: final_dmg_c,
        damage_total: final_dmg_t,
        runtime_recipes: if player.lite_mode { Vec::new() } else { eff_recipes.clone() },
        runtime_stats: if player.lite_mode { None } else { final_rt.clone() },
        override_attack_coeff: None,
        applied_recipes: if player.lite_mode { Vec::new() } else {
            // 主体 cast 事件填 applied_recipes（与 main.rs 主路径一致）
            let base_name = actual_skill.name.split('·').next().unwrap_or(&actual_skill.name);
            crate::collect_recipes_indexed(player, actual_skill.skill_id, base_name,
                                            &eff_recipes, ctx.recipes_table)
    ...
    // 7. 脚本 emit 出的子事件补伤害
    for mut ev in extra {
        crate::fill_event_damage(&mut ev, ctx.skill_by_id, ctx.dmg_ctx, ctx.recipes_table, player, &eff_recipes);
        out.push(ev);
    }

    // 8. flush_advance
    let mut advance_events = player.flush_advance();
    crate::fill_tick_events(&mut advance_events, ctx.skill_by_id, ctx.dmg_ctx, ctx.recipes_table, player);
    ...
    *prev_time = player.current_time;
    ...
    CastOutcome { events: out, cast_success: true }
}
...
pub fn simulate_macro(
    ...
    pauses: &[(f64, f64)],
) -> (Vec<CastEvent>, Vec<MacroStepDebug>) {
    let _t0 = std::time::Instant::now();
    let _guard = crate::scopeguard_perf(
        |ns| crate::perf_add(|p| { p.macro_eval_n += 1; p.macro_eval_ns += ns; }),
        _t0
    );
    // 停手窗口：把 (start, dur) 转成 (start, end)，按 start 排序
    let mut pause_ranges: Vec<(f64, f64)> = pauses.iter()
        .map(|&(s, d)| (s, s + d))
    ...
    let skip_pause = |player: &mut Player, prev_time: &mut f64, timeline: &mut Vec<CastEvent>,
                      skill_by_id: &HashMap<u32, &SkillSpec>, dmg_ctx: Option<&(crate::Attributes, crate::TargetConfig)>,
                      recipes_table: &[RecipeEntry], pauses: &[(f64, f64)]| {
        for &(ps, pe) in pauses {
    ...
    let mut last_busy_end: f64 = player.active_cds.iter()
        .filter(|(k, _)| k.starts_with("gcd_"))
        .map(|(_, &v)| v)
    ...
    let bufftime_thresholds: Vec<(u32, f64, bool)> = bufftime_str.iter()
        .filter_map(|(n, v, is_t)| buff_name_to_id(n).map(|id| (id, *v, *is_t)))
        .collect();

    for _ in 0..safety_limit {
        if cast_count >= max_casts { break; }
        if player.current_time > max_duration { break; }

        // 停手：当前时间落在暂停窗口内 → 跳到窗口结束（期间只跑 buff tick）
        skip_pause(player, prev_time, &mut timeline, skill_by_id, dmg_ctx, recipes_table, &pause_ranges);
        ...
        let (pool, new_last, _p1_debug) = evaluate_phase1(
            &config.pages[page_idx], player, skill_map, skill_by_id, last_skill.clone(), false,
        );
        let _ns_p1 = _t_p1.elapsed().as_nanos() as u64;
        crate::perf_add(|p| { p.macro_phase1_n += 1; p.macro_phase1_ns += _ns_p1; });
        ...
        if pool.is_empty() {
            ...
            crate::perf_add(|p| { p.macro_adv_next_n += 1; p.macro_adv_next_ns += _ns_next; });
            let target = if next_event.is_finite() { next_event } else { player.current_time + min_advance };
            ...
            crate::perf_add(|p| { p.macro_adv_ticks_n += 1; p.macro_adv_ticks_ns += _ns_ticks; });
            let _t_fill = std::time::Instant::now();
            crate::fill_tick_events(&mut tick_events, skill_by_id, dmg_ctx, recipes_table, player);
            let _ns_fill = _t_fill.elapsed().as_nanos() as u64;
            crate::perf_add(|p| { p.macro_adv_fill_n += 1; p.macro_adv_fill_ns += _ns_fill; });
            timeline.extend(tick_events);
            *prev_time = target;
            player.current_time = target;
            let _ns_adv = _t_adv.elapsed().as_nanos() as u64;
            crate::perf_add(|p| { p.macro_advance_n += 1; p.macro_advance_ns += _ns_adv; });
            continue;
        }

        // 阶段二：选出第一个可释放的技能（debug 关闭，同上）
        let _t_p2 = std::time::Instant::now();
        let (phase2_result, _p2_debug) = evaluate_phase2(&pool, player, skill_map, skill_by_id, false);
        let _ns_p2 = _t_p2.elapsed().as_nanos() as u64;
        crate::perf_add(|p| { p.macro_phase2_n += 1; p.macro_phase2_ns += _ns_p2; });
        ...
        let actual_skill = crate::resolve_combo_follow(result.skill, player, skill_by_id)
            .unwrap_or(result.skill);
        ...
        let outcome = execute_cast(
            player, actual_skill,
            prev_time, is_first_main, &mut last_busy_end,
            &cast_ctx,
        );
        timeline.extend(outcome.events);
        if outcome.cast_success {
            // 更新 last_skill 为实际释放的技能（跨轮次持久化）
            last_skill = Some(actual_skill.name.split('·').next()
                .unwrap_or(&actual_skill.name).to_string());
            cast_count += 1;
        }
    }
