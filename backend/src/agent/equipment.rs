...
pub fn inspect_workspace(
    ...
    let focused_candidate = workspace.focus.as_ref().and_then(|focus| {
        item_summary(runtime, &focus.position, focus.candidate_id)
    });
    EquipmentWorkspaceSummaryV1 {
...
pub fn search_catalog(
    ...
    let wants_set = normalized.contains("套装") || normalized.contains("四件套") || normalized.contains("4件套");
    let mut items = runtime
        .equipment_items()
        .filter_map(|item| {
            let position = subtype_position(item.sub_type)?;
            if query.position.as_deref().is_some_and(|value| value != position) {
                return None;
            }
            let set_name = (item.set_id > 0)
                .then(|| runtime.equipment_set_name(item.set_id))
                .flatten();
            let haystack = format!("{} {} {} {}", item.name, item.magic_type, item.belong_school, set_name.as_deref().unwrap_or(""));
            let matches = if wants_qiegao {
                set_name.as_deref().is_some_and(|name| name.contains("切糕"))
            } else if wants_set {
                item.set_id > 0 && !set_name.as_deref().is_some_and(|name| name.contains("切糕"))
            } else {
        ...
        .collect::<Vec<_>>();
    items.sort_by(|left, right| right.level.cmp(&left.level).then_with(|| left.name.cmp(&right.name)));
    items.truncate(16);
    items
}
...
pub fn compare_focus(
...
) -> Result<(EvidenceEnvelopeV1<EquipmentComparisonEvidenceV1>, EquipmentComparisonPresentationV1), ToolError> {
    let focus = workspace.focus.as_ref().ok_or(ToolError::EquipmentFocusUnavailable)?;
    let current = workspace.slots.get(&focus.position).ok_or(ToolError::EquipmentFocusUnavailable)?;
    ...
    let before_calc = runtime.calculate_equipment(&workspace.slots, workspace.stone_id, &baseline.simulation.talents);
    let after_calc = runtime.calculate_equipment(&candidate_slots, workspace.stone_id, &baseline.simulation.talents);
    let baseline_snapshot = snapshot_with_build(baseline, runtime, &workspace.slots, &before_calc.raw)?;
    let candidate_snapshot = snapshot_with_build(baseline, runtime, &candidate_slots, &after_calc.raw)?;
    let context = runtime.context();
    let before_sim = simulate_scenario(trace_id, &baseline_snapshot, &context, runtime.provenance(), budget)?;
    let after_sim = simulate_scenario(trace_id, &candidate_snapshot, &context, runtime.provenance(), budget)?;
    ...
    let presentation = EquipmentComparisonPresentationV1 {
        title: format!("{}：{} → {}", focus.position, before_item.name, after_item.name),
        ...
        dps_delta_percent: if before_dps.abs() > f64::EPSILON { (after_dps / before_dps - 1.0) * 100.0 } else { 0.0 },
        source_label: workspace.source_label.clone(),
    };
...
pub fn compare_strategies(
    ...
    let (qiegao_name, qiegao_slots, qiegao_items) = build_four_piece(runtime, workspace, true)
        .ok_or(ToolError::EquipmentStrategyUnavailable)?;
    let set_calc = runtime.calculate_equipment(&set_slots, workspace.stone_id, &baseline.simulation.talents);
    let qiegao_calc = runtime.calculate_equipment(&qiegao_slots, workspace.stone_id, &baseline.simulation.talents);
    ...
    let context = runtime.context();
    let set_sim = simulate_scenario(trace_id, &set_snapshot, &context, runtime.provenance(), budget)?;
    let qiegao_sim = simulate_scenario(trace_id, &qiegao_snapshot, &context, runtime.provenance(), budget)?;
    let before_dps = set_sim.evidence.result.dps;
    let after_dps = qiegao_sim.evidence.result.dps;
    let presentation = EquipmentComparisonPresentationV1 {
        title: "四件套 vs 四切糕".to_string(), position: "MULTI".to_string(),
        before_item: format!("四件套 · {set_name}"), after_item: format!("四切糕 · {qiegao_name}"),
        panel_rows: panel_rows(&set_calc, &qiegao_calc), before_dps, after_dps,
        dps_delta: after_dps - before_dps,
        dps_delta_percent: if before_dps.abs() > f64::EPSILON { (after_dps / before_dps - 1.0) * 100.0 } else { 0.0 },
        source_label: workspace.source_label.clone(),
    };
    let evidence = EvidenceEnvelopeV1::new(
        trace_id, COMPARE_EQUIPMENT_STRATEGIES, &baseline.scenario_hash,
        serde_json::json!({"strategy":"four_set_vs_four_qiegao"}),
        EquipmentComparisonEvidenceV1 {
            comparison: presentation.clone(), before_equipment: set_items, after_equipment: qiegao_items,
            baseline: set_sim.evidence.result, candidate: qiegao_sim.evidence.result,
...
fn build_four_piece(
    ...
    workspace: &EquipmentWorkspaceV1,
    qiegao: bool,
) -> Option<(String, HashMap<String, equip::SlotConfig>, Vec<EquipmentItemSummaryV1>)> {
    ...
    for item in runtime.equipment_items() {
        let Some(position) = subtype_position(item.sub_type) else { continue };
        if !armor.contains(&position) || !workspace.slots.contains_key(position) || item.set_id == 0 { continue; }
        if item.magic_type.contains("(PVP)") || item.magic_type.contains("(PVX)") { continue; }
        if !item_matches_mount(runtime.mount(), item) { continue; }
        let set_name = runtime.equipment_set_name(item.set_id).unwrap_or_default();
        if set_name.contains("切糕") != qiegao { continue; }
        groups.entry(item.set_id).or_default().push((position, item));
    }
    let mut candidates = groups.into_iter().filter_map(|(set_id, entries)| {
        let mut best = BTreeMap::<String, &equip::EquipItem>::new();
        for (position, item) in entries {
            let replace = best.get(position).is_none_or(|current| item.level > current.level);
            if replace { best.insert(position.to_string(), item); }
        }
        if best.len() < 4 { return None; }
    ...
    Some((runtime.equipment_set_name(set_id).unwrap_or_else(|| format!("套装#{set_id}")), slots, summaries))
}
...
fn equipment_map(slots: &HashMap<String, equip::SlotConfig>) -> HashMap<String, u32> {
    ...
    for (position, slot) in slots {
        if slot.equip_id > 0 { result.insert(position.clone(), slot.equip_id); }
        if slot.enchant_id > 0 { result.insert(format!("ENCHANT_{position}"), slot.enchant_id); }
    }
    result
}
...
fn item_summary(runtime: &AgentRuntime, position: &str, id: u32) -> Option<EquipmentItemSummaryV1> {
    ...
    runtime.equipment_item(subtype, id).map(|item| item_summary_from_item(runtime, position, item))
}

fn item_summary_from_item(runtime: &AgentRuntime, position: &str, item: &equip::EquipItem) -> EquipmentItemSummaryV1 {
    let set_name = (item.set_id > 0).then(|| runtime.equipment_set_name(item.set_id)).flatten();
    let category = if set_name.as_deref().is_some_and(|name| name.contains("切糕")) {
        "切糕"
    ...
    EquipmentItemSummaryV1 {
        position: position.to_string(), id: item.id, name: item.name.clone(), level: item.level,
        quality: item.quality, set_name, category: category.to_string(),
...
fn panel_map(calc: &equip::CalcResponse) -> BTreeMap<String, f64> {
    let p = &calc.panel;
    BTreeMap::from([
        ("score".into(), calc.score as f64), ("attack".into(), p.physics_attack_power),
        ("crit".into(), p.crit_rate * 100.0), ("crit_effect".into(), p.crit_effect * 100.0),
        ("overcome".into(), p.overcome_rate * 100.0), ("strain".into(), p.strain_rate * 100.0),
        ("haste".into(), p.haste_rate * 100.0), ("surplus".into(), p.surplus_value),
        ("agility".into(), p.agility), ("strength".into(), p.strength), ("vitality".into(), p.vitality),
    ])
}

fn panel_rows(before: &equip::CalcResponse, after: &equip::CalcResponse) -> Vec<EquipmentPanelRowV1> {
    let b = panel_map(before); let a = panel_map(after);
    let labels = [("score","装分",""),("attack","外功攻击",""),("crit","会心","%"),("crit_effect","会心效果","%"),("overcome","破防","%"),("strain","无双","%"),("haste","加速","%"),("surplus","破招",""),("agility","身法",""),("strength","力道","")];
    labels.into_iter().map(|(key,label,unit)| {
        let before = *b.get(key).unwrap_or(&0.0); let after = *a.get(key).unwrap_or(&0.0);
        EquipmentPanelRowV1 { key:key.into(), label:label.into(), unit:unit.into(), before, after, delta:after-before }
    }).collect()
}

fn position_subtype(position: &str) -> Option<u8> {
    Some(match position { "PRIMARY_WEAPON"=>0,"SECONDARY_WEAPON"=>1,"NECKLACE"=>4,"RING_1"|"RING_2"=>5,"PENDANT"=>7,"HAT"=>3,"JACKET"=>2,"BELT"=>6,"WRIST"=>10,"BOTTOMS"=>8,"SHOES"=>9,_=>return None })
}
fn subtype_position(subtype: u8) -> Option<&'static str> {
    Some(match subtype {0=>"PRIMARY_WEAPON",1=>"SECONDARY_WEAPON",2=>"JACKET",3=>"HAT",4=>"NECKLACE",5=>"RING_1",6=>"BELT",7=>"PENDANT",8=>"BOTTOMS",9=>"SHOES",10=>"WRIST",_=>return None})
}
...
mod tests {
    ...
    fn equipment_positions_match_the_existing_equipment_page_contract() {
        let expected = [
            ("PRIMARY_WEAPON", 0), ("SECONDARY_WEAPON", 1), ("JACKET", 2),
            ("HAT", 3), ("NECKLACE", 4), ("RING_1", 5), ("BELT", 6),
            ("PENDANT", 7), ("BOTTOMS", 8), ("SHOES", 9), ("WRIST", 10),
        ];
