...
pub const AGENT_PROMPT_VERSION: &str = "agent-system/v30";
const AGENT_SYSTEM_PROMPT: &str = include_str!("../../prompts/agent_system_v30.md");
...
mod tests {
    ...
    fn current_prompt_is_single_model_led_instruction() {
        ...
        assert_eq!(prompt.version, "agent-system/v30");
        ...
        assert!(prompt.instructions.contains("判断先行"));
        assert!(prompt.instructions.contains("[[显示文字|op:操作编号]]"));
        assert!(prompt.instructions.len() < 8_000);
    }
}
