...
const DUN_DANG_COEFS: [f64; 10] = [
    41.0, 82.0, 123.0, 164.0, 205.0,
    246.0, 287.0, 328.0, 369.0, 410.0,
];

/// 千山盾挡（8448）level 1~10 的 cof（约为普通版 1.25x，各级独立取值）
const DUN_DANG_QIAN_SHAN_COEFS: [f64; 10] = [
    51.0, 102.0, 154.0, 205.0, 256.0,
    307.0, 358.0, 410.0, 461.0, 512.0,
];
...
pub fn cast_skill(player: &mut Player, _em: &mut ScriptEmitter, _t: f64) {
    if player.mount != Mount::TieGuYi { return; }
    ...
    let (buff_id, cof) = if player.has_talent(13421) {
        (BUFF_DUN_DANG_QIAN_SHAN, DUN_DANG_QIAN_SHAN_COEFS[(tier - 1) as usize])
    } else {
    ...
    player.bind_buff_effects(buff_id, vec![
        EffectEntry { field: AttribField::VitalityToParryValueCof, value: cof },
    ]);
