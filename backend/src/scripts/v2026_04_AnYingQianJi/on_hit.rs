...
pub fn on_player_hit(player: &mut Player, t: f64) -> Vec<CastEvent> {
    ...
    // ── 苍云阵·6重横绝（自己开阵）：被攻击 → +102 攻击百分比 5s ──
    // buff.txt 8404 描述"外功基础攻击力提高10%"
    if formation_is_self(player, "cangyun") {
        player.add_buff(BUFF_HENG_JUE);
    }

    // ── 铁骨阵·4重格挡恢复（自己开阵）：被攻击 +5 格挡，30s 内置 CD（用户拍板，不挂 buff）──
    if formation_is_self(player, "tiegu") {
