...
它负责拆解问题、提出假设和组织证据，不直接生成“看起来合理”的伤害数字。当前闭环包括 provider adapter、领域 Playbook、弹性但有预算的工具循环、数值证据校验、Run API/SSE、append-only 持久会话、有界可见上下文恢复和面试用实验面板。当前 `agent-system/v16` 由服务端预取不可变场景与任务相关的当前版本资料，并按证据缺口选择时间轴或同场景实验；宏输入直接向模型暴露模拟器解析出的条件 AST 和全括号语义，模型不得自行重组 `&`、`|`。涉及无界端时服务端仍只暴露知识检索，不允许形成本项目模拟结论。Phase 1 工具评测保持 20/20；Phase 2 离线模型级评测为 12/12。DeepSeek V4 Pro 固定十题从 1/10 提升到 6/10，复跑为 5/10，三轮越权工具调用均为 0；真实结果不包装成稳定准确率。
