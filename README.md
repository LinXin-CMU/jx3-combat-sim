...
它负责拆解问题、提出假设和组织证据，不直接生成“看起来合理”的伤害数字。当前闭环包括 provider adapter、领域 Playbook、弹性但有预算的工具循环、数值证据校验、Run API/SSE、append-only 持久会话、有界可见上下文恢复和面试用实验面板。当前 `agent-system/v17` 将循环分析约束为“版本与输入口径 → 攻略依据 → 基线时间轴诊断 → 明确优点与风险 → 按需提出候选 → 同场景 A/B → 手动或宏方案”：服务端在诊断前不开放候选比较，诊断型问题也不会被强行导向修改。宏输入继续向模型暴露模拟器解析出的条件 AST 和全括号语义，模型不得自行重组 `&`、`|`。涉及无界端时服务端仍只暴露知识检索，不允许形成本项目模拟结论。Phase 1 工具评测与 Phase 2 离线模型级评测均需在每次协议升级后复跑；真实模型结果不包装成稳定准确率。
