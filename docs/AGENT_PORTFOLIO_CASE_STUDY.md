...
| 固定知识检索 Recall@5 | 23/23（100%，门槛 90%） |
| 知识版本/来源资格/拒答断言 | 27/27 |
| Embedded 混合检索状态 | hybrid_rrf / cache hit / 无降级 |
| Dense 增益对照 | BM25 Top 5 漏召回 → Hybrid Top 5 命中 |
| Embedded 固定集耗时 | 模型缓存命中 + 27 题共 3.10s |
| 版本越界与伪造来源进入报告 | 0 |
| 确定性工具评测 | 20/20 |
| Agent 模型层离线评测 | 12/12 |
...
- Embedded 首次真评测又发现乱码问题以 0.429806 相似度误召回宏文档；系统没有用 BM25 降级后的 23/23 冒充 Dense 通过。最终采用 0.45 基础阈值、0.55 纯 Dense 入池门槛和加权 RRF，在保持 23/23 与 27 条安全断言的同时，新增一条 BM25 Top 5 漏召回而 Hybrid 恢复的语义对照。
...
早期真实模型结果见 [`baselines/2026-08-26-agent-deepseek-v4-pro.md`](baselines/2026-08-26-agent-deepseek-v4-pro.md)，K5B 分层结果见 [`baselines/2026-08-27-agent-k5b-real-model.md`](baselines/2026-08-27-agent-k5b-real-model.md)，领域 Playbook 结果见 [`baselines/2026-08-28-agent-domain-playbooks-v13.md`](baselines/2026-08-28-agent-domain-playbooks-v13.md)。下一步是扩充可验证领域 Claim 与派生观测，并在录屏中同时展示专业回答、安全降级和任务工具边界。
