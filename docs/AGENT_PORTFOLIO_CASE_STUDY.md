...
- `agent-system/v17` 把“先诊断、后干预”从提示词建议升级为服务端状态机。时间轴先产出带证据路径与解释边界的循环诊断画像；候选比较只在诊断完成且用户明确要求优化时开放。手动循环与宏循环采用同一证据门槛，任何可执行改法都必须同时引用基线诊断与同场景比较。
- `agent-system/v18` 增加账号隔离的本地存档访问：模型只能按显示名称检索宏、循环、配装、属性与战斗广场方案，再用内容寻址 ID 精确读取。双宏对比固定当前配装、奇穴、秘籍、目标、延迟与团队环境，只替换宏并调用确定性模拟器；完整方案对比则显式保留环境差异。工具不接收文件路径，也不暴露设置、凭据、会话或缓存。
