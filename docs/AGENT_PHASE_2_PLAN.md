...
状态：P2-00 至 P2-07 已完成，P2-08 的工程与文档收口已完成，最终录屏待用户执行。真实 Agent 会话已在用户确认后增量写入，原有 userdata 内容与哈希保持不变；DeepSeek V4 Pro 已在 USD 0.70 硬预算内完成固定评测和真实多轮续接。公网仍停在阻断节点 C。完整结果见 [`baselines/2026-08-26-agent-deepseek-v4-pro.md`](baselines/2026-08-26-agent-deepseek-v4-pro.md)。
...
离线结果：12/12 通过，覆盖 6 个自然语言任务、2 个提示注入、2 个授权边界与 2 个续接轮次；证据引用率与工具边界通过率均为 100%。真实固定十题三轮结果为 1/10、6/10、5/10，三轮越权工具调用均为 0；波动和失败样本如实保留。另以真实 session 完成一次跨轮续接并重新取证。详见真实模型基线。

### P2-08：作品集收口

状态：工程、评测与文档已完成；最终 90 秒录屏待用户执行。
...
3. P2-01/P2-02 先完成离线和 mock 测试；用户随后确认 DeepSeek V4 Pro、服务端环境变量注入与 USD 0.70 开发费用上限。

阻断节点 A、B 均已确认并验收：真实模型评测、真实多轮续接和 userdata 写保护均已完成。阻断节点 C 仍保持关闭，私有仓库和本地访问不自动扩展为公网演示。
