...
状态：P2-00 至 P2-06 已完成，P2-07/P2-08 的离线部分已完成。真实 Agent 会话已在用户确认后增量写入，原有 userdata 内容与哈希保持不变。尚未调用真实模型，当前停在阻断节点 A；完整离线基线见 [`baselines/2026-08-26-agent-phase2-offline.md`](baselines/2026-08-26-agent-phase2-offline.md)。
...
结果：用户已于 2026-08-26 确认。写入前旧数据为 178 个文件、863,482 bytes，清单聚合 SHA256 为 `f4395818602446c985a0442eb6d8dbf082b445dec9767334d3024d75837d2e2a`；写入后排除新增 `agent_sessions` 的旧数据数量、字节数和哈希完全一致。新增会话默认保留，不自动删除。
...
离线结果：12/12 通过，覆盖 6 个自然语言任务、2 个提示注入、2 个授权边界与 2 个续接轮次；证据引用率与工具边界通过率均为 100%。该结果只证明 Orchestrator、会话和评测管线，不声称真实模型回答质量。真实模型启用后须在同一 runner 中补齐 token、成本、成功与失败样本。

### P2-08：作品集收口

状态：离线工程收口已完成；真实模型结果与最终录屏待补。
...
离线交付：已提供一键验收脚本、案例页和 90 秒演示脚本；Rust 113/113、Phase 1 20/20、Phase 2 12/12，真实 userdata 写保护通过，公网保持关闭。
...
P2-01 至 P2-06 及 P2-07/P2-08 的离线交付已按上述边界完成。阻断节点 B 已在写入前确认并通过写后哈希复核；首次真实模型调用的阻断节点 A 现在是唯一的下一步。用户确认具体 profile/model 与费用上限后，才能运行真实评测。阻断节点 C 仍保持关闭，私有仓库和本地访问不自动扩展为公网演示。
