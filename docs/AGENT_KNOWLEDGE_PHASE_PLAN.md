...
状态：K1–K4 已完成并部署到本地 3005；K5 离线检索与端到端验收已完成，真实模型分层评测和作品集最终叙事待确认。
...
一键隔离验收现同时覆盖 146 个 Rust 测试、20 个工具题、12 个 Agent 离线题、知识来源卡 HTTP 闭环，以及“资料假设 → 强类型候选 → 确定性 A/B”的双证据测试；使用临时 Cargo target、端口与 userdata，不需要停止 3005，也不改动真实会话。完整证据见 [`baselines/2026-08-26-agent-knowledge-k5.md`](baselines/2026-08-26-agent-knowledge-k5.md)。
