...
状态：K1–K5 已完成并部署到本地 3005；真实模型分层基线已冻结，作品集最终叙事与专业表达优化待确认。
...
Prompt v6 与 Orchestrator 已按问题选择最小闭环：
...
离线部分已完成：固定集包含 20 个 Recall@5 正例和 4 个拒答/安全用例，Recall@5 为 100%，25 条版本、来源资格和拒答断言全部通过。评测首次运行发现“当前赛季加权会让零重叠问题产生伪结果”，修复后无答案查询稳定返回空结果，正例仍全部 Top 1。

一键隔离验收现同时覆盖 150 个 Rust 测试、20 个工具题、12 个 Agent 离线题、知识来源卡 HTTP 闭环，以及“资料假设 → 强类型候选 → 确定性 A/B”的双证据测试；使用临时 Cargo target、端口与 userdata，不需要停止 3005，也不改动真实会话。完整离线证据见 [`baselines/2026-08-26-agent-knowledge-k5.md`](baselines/2026-08-26-agent-knowledge-k5.md)。
