...
升级 Prompt 至 v5，并增加版本化报告协议。报告新增结构化 `sources`，而不是让模型在自然语言里随意拼链接：
...
服务端必须校验这些字段与本次检索证据完全一致。模型伪造 URL、引用本次 run 之外的资料、把失败页面当正文或把历史赛季说成当前版本，均拒绝或降级。

同时收紧数值证据白名单：`metrics` 只能引用模拟、对比或时间轴工具的数值字段，不能引用检索评分、文档中的攻略数字或模型计算结果。
