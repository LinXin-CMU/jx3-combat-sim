...
日期：2026-08-26  
范围：`agent-system/v4`、可信场景预取、逐条报告校验、DeepSeek V4 Flash Profile
公网：关闭
...
7. V4 Flash 在完成一个领域工具后进入无工具报告轮次，从而启用 DeepSeek JSON mode，减少因报告结构损坏产生的第三次请求。
8. 数值校验允许已验证 metric 标签中的固有数字，但不会把“50 怒”误用为“提升 50%”的证据。降级时使用完整可读备用句，不再在原句中插入大量占位符。
