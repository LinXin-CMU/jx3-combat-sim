...
### C-001：赴敌距离公式内部矛盾（已解决）

原文曾同时出现“取两个距离的最大值”、`MIN（突进保护距离，4尺）` 和 5.5 尺示例，内部矛盾，因此运行时没有把旧公式作为事实。

2026-08-28 作者同步修正《苍云进阶机制（2025）》和《暗影千机·分山劲白皮书》，当前公式为：

```text
实际不触发突进的距离 = MAX（突进保护距离，4尺）
```

该公式现在与文字和示例一致。C-001 从 `unresolved_internal` 关闭，并形成来源绑定 Claim `fs-charge-001`；Boss 距离补偿仍未进入木桩模拟器，因此属于可引用知识事实，不作为本轮模拟指标。
