...
- 已验证基线：`cargo test` 为 109 个测试通过；前端可用 `node --check frontend/app.js` 做语法检查。
...
- Agent P2 provider 基线：`docs/baselines/2026-08-25-agent-provider-layer.md` 记录 P2-01/P2-02 的 adapter、凭据、mock、隔离 HTTP 与完整回归；当前仍未调用真实模型或写入 Agent 会话。
- Agent P2 编排基线：`docs/baselines/2026-08-25-agent-orchestrator.md` 记录 prompt、四工具循环、报告和数值证据校验；`docs/baselines/2026-08-26-agent-run-lifecycle.md` 记录 Run API、SSE、取消、并发与 Router 代理回归。当前仍未调用真实模型或写入 Agent 会话。
