...
- 已验证基线：`cargo test` 为 146 个测试通过；前端可用 `node --check frontend/app.js` 与 `node --check frontend/agent.js` 做语法检查。
...
- Agent P2 离线基线：`docs/baselines/2026-08-26-agent-phase2-offline.md` 保留当时 113 项 Rust 测试的历史快照；当前为 146 项、20/20 工具评测、12/12 模型级离线评测。真实 Agent 会话已在用户确认后仅增量写入 `agent_sessions/v1`，既有 178 个文件及其清单哈希不变。
- Agent 知识基线：本地 Vault 为 159 篇文档、3796 分块、13 个赛季；固定集 Recall@5 为 20/20，25 条版本/资格/拒答断言通过。`tools/agent-phase2-verify.ps1` 使用隔离 target、端口和 userdata 复现完整离线验收，详见 `docs/baselines/2026-08-26-agent-knowledge-k5.md`。
